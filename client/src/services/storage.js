...
export const storage = {
  ...
  getCurrentUser: () => {
    ...
    try {
      return (
        JSON.parse(localStorage.getItem("token_sys_session")) || { jwt }
      );
    } catch (e) {
      return { jwt };
    }
  ...
  getTokens: async (eventId) => {
    const res = await fetch(
      `${API_URL}/api/tokens?eventId=${eventId}`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  joinEvent: async (eventId, attendeeName) => {
    const res = await fetch(
      `${API_URL}/api/public/events/${eventId}/join`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  updateTokenStatus: async (tokenId, status) => {
    ...
    if (!res.ok) {
      throw new Error(
        data.message || "Failed to update token status"
      );
    }
